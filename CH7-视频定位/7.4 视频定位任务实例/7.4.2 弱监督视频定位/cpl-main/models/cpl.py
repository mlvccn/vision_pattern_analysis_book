...
class CPL(nn.Module):
    ...
    def _mask_words(self, words_feat, words_len, weights=None):
        """
        对输入的单词特征进行掩码操作，模拟掩码语言模型中的掩码过程。

        参数:
        words_feat (torch.Tensor): 输入的单词特征张量，形状为 (batch_size, seq_len, feature_dim)。
        words_len (torch.Tensor): 每个样本的实际单词长度，形状为 (batch_size,)。
        weights (torch.Tensor, 可选): 用于随机选择掩码位置的权重，形状为 (batch_size, seq_len)。默认为 None。

        返回:
        tuple: 包含两个元素的元组，第一个元素是掩码后的单词特征张量，第二个元素是掩码位置的布尔张量。
        """
        # 将掩码向量移动到 GPU 并调整形状
