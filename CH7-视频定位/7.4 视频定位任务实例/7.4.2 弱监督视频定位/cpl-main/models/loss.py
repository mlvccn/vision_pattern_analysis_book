...
def rec_loss(words_logit, words_id, words_mask, num_props, ref_words_logit=None, **kwargs):
    ...
    参数:
    words_logit (torch.Tensor): 单词的对数概率。
    words_id (torch.Tensor): 单词的ID。
    words_mask (torch.Tensor): 单词的掩码。
    num_props (int): 提议的数量。
    ref_words_logit (torch.Tensor, 可选): 参考单词的对数概率。默认值为None。

    返回:
    torch.Tensor: 最终的重构损失。
    """
    # 计算批次大小
