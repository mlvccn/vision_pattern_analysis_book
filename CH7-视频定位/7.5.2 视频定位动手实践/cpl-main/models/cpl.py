...
class CPL(nn.Module):
    def __init__(self, config):
        ...
        # 新增: 时序注意力模块 默认不启用
        self.use_temporal_attn = config.get('use_temporal_attn', False)
        if self.use_temporal_attn:
            self.temporal_attn = TemporalAttentionModule(
                hidden_size=config['hidden_size'],
                num_heads=config.get('temporal_attn_heads', 4),
    ...
    def forward(self, frames_feat, frames_len, words_id, words_feat, words_len, weights, **kwargs):
        ...
        frames_feat = torch.cat([frames_feat, pred_vec], dim=1)     #添加可学习token
        frames_feat = F.dropout(frames_feat, self.dropout, self.training)
        frames_feat = self.frame_fc(frames_feat)                    #线性投影
        frames_mask = _generate_mask(frames_feat, frames_len)

        # 新增: 应用时序注意力模块
        if self.use_temporal_attn:
            frames_feat = self.temporal_attn(frames_feat, frames_mask)
...
class TemporalAttentionModule(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        attn_output, _ = self.self_attn(
            x, x, x, key_padding_mask=~mask.bool() if mask is not None else None
        )
        ...
        return x.transpose(0, 1)  # [batch_size, seq_len, hidden_size]
