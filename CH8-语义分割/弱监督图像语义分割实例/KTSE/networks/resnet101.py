...
class ResBlock_ysh(nn.Module):
    def __init__(self, in_chan, out_chan, stride, ks, dilation):
        super(ResBlock_ysh, self).__init__()
        if stride > 1:
            raise NotImplementedError("Stride > 1 not supported")
        self.convbnrelu1 = ConvBnReLU(in_chan, out_chan, 3, stride=stride, pd=1)
        self.convbnrelu2 = ConvBnReLU(out_chan, out_chan, 3, stride=stride, pd=dilation, dilation=dilation)

        self.branch = nn.Sequential(nn.Conv2d(in_chan, out_chan, 1, 1, 0, bias=False))
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.conv1x1 = nn.Conv2d(2048,20,1,1,0)
