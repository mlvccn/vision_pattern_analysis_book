...
from .utils import (
    ...
    get_qwen_masks_and_logit_probs,
...
class ImageAttr_QWEN:
    ...
    def from_pretrained(
        ...
        **kwargs: Dict[str, Any],
        ...
        return cls(model, tokenizer, model_name, processor, **kwargs)
    ...
    def _output(self):
        if self._cache.get("output") is None:
            ...
            inputs = self.processor(
                    text=[prompt],
                    images=self.partitioner.get_image(),
                    padding=True,
                    return_tensors="pt",
            ...
            print(prompt)
            print(response)
            self._cache["output"] = prompt + response[0]
