...
Then install this package from the repo root:
...
A note for hardware: all experiments we run use one or multiple NVIDIA A100 GPUs, which have 80G memory per chip, if you meet the OOM issue, please refer to the [pull requests](https://github.com/llm-attacks/llm-attacks/pull/77), then you can use a single GPU with 24G memory and reduce the batch size to run the Demo.
...
We would like to thank the authors of [gcg]( https://github.com/llm-attacks/llm-attacks) which has significantly accelerated the development of our book
