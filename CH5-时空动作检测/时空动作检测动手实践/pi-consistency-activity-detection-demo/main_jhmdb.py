...
def train_model_interface(args, label_minibatch, unlabel_minibatch, epoch, wt_ramp):
    ...
    # labeled_seg_data = concat_seg[labeled_vid_index] # 修改
    labeled_seg_data = concat_seg.cuda()[labeled_vid_index]
...
def validate(model, val_data_loader, epoch):
    ...
    info_loss = {
        'val_loss': r_total,
        'val_loss_seg': r_seg,
        'val_loss_cls': r_class,
        'val_acc': r_acc,
        'val_IOU': average_IOU
    }
    writer.add_scalars('val/loss', info_loss, epoch)
    #=======修改，增加tensorboard===========
    sys.stdout.flush()
    return r_total
...
def parse_args():
    ...
    parser.add_argument('--txt_file_label', type=str, default='jhmdb_classes_list_per_20_labeled.txt', help='label subset')
    parser.add_argument('--txt_file_unlabel', type=str, default='jhmdb_classes_list_per_80_unlabeled.txt', help='unlabele subset')
    parser.add_argument('--txt_test_anno', type=str, default='testlist.txt', help='test annotation')#增加
    ...
    parser.add_argument('--log_dir', type=str, default='result/jhmdb', help='base directory for saving logs and weights')
...
if __name__ == '__main__':
    ...
    from datasets.jhmdb_dataloader import JHMDB
    
    labeled_trainset = JHMDB('train', [224, 224], file_id=args.txt_file_label, use_random_start_frame=False)
    unlabeled_trainset = JHMDB('train', [224, 224], file_id=args.txt_file_unlabel, use_random_start_frame=False)
    ...
    from models.capsules_jhmdb import CapsNet
    ...
    save_path = os.path.join(log_dir, exp_id)
