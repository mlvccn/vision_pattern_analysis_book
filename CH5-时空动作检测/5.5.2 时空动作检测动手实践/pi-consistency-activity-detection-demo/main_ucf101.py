...
def train_model_interface(args, label_minibatch, unlabel_minibatch, epoch, wt_ramp):
    ...
    sup_vid_labels = label_minibatch['label_vid'] #sup_vid_labels: [1,1,1,1]
    unsup_vid_labels = unlabel_minibatch['label_vid'] #unsup_vid_labels: [0,0,0,0]
    ...
    output, predicted_action, feat = model(concat_data, concat_action, concat_labels, epoch, args.thresh_epoch) #output: (8, 1, 8, 224, 224) predicted_action: (8, 24)
    flip_op, _, _ = model(concat_fl_data, concat_action, concat_labels, epoch, args.thresh_epoch) #数据增强 flip_op: (8, 1, 8, 224, 224)
...
def parse_args():
    ...
    parser.add_argument('--log_dir', type=str, default='result/ucf101-24', help='base directory for saving logs and weights')
...
if __name__ == '__main__':
    ...
    unlabeled_trainset = UCF101DataLoader('train', [224, 224], file_id=args.pkl_file_label, use_random_start_frame=False)
    validationset = UCF101DataLoader('validation',[224, 224], file_id="testing_annots.pkl", use_random_start_frame=False)
