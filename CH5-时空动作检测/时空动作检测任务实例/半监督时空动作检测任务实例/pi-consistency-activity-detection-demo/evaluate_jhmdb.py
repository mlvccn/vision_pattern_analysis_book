...
def iou():
    ...
    for saved_wts in sorted_weights:
        ...
        with torch.no_grad():
            ...
            for idx, sample in enumerate(tqdm(val_data_loader, desc=f'Evaluating test set', unit='video')):
            ...
            with open(output_file, 'a') as f:
                # 将模型权重路径写入文件
                f.write(f"Model Weights: {saved_wts}\n")
                ...
                # 如果您还想在控制台打印结果，可以保留原来的 print 语句
                print(f"Model Weights: {saved_wts}")
                print(f"Accuracy: {n_correct / np.sum(n_vids):.4f}, IoU/fmap/vmap: {iou_threshs[4]:.2f} {fmAP[4]:.4f} {vmAP[4]:.4f}, {iou_threshs[10]:.2f} {fmAP[10]:.4f} {vmAP[10]:.4f}")
