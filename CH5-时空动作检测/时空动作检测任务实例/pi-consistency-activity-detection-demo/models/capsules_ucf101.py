...
class CapsNet(nn.Module):
    def __init__(self, pt_path='weights/rgb_charades.pt', P=4, pretrained_load='i3d'):
        ...
        参数:
            pt_path: 预训练权重路径
            P: 姿态矩阵的维度(P x P)
            pretrained_load: 预训练模型类型
        """
        super(CapsNet, self).__init__()
        self.P = P  # 姿态矩阵维度

        # 初始化I3D卷积网络(加载到Mixed_4f层)
        ...
        # 定义网络各层
        self.primary_caps = PrimaryCaps(832, 32, 9, P, stride=1)  # 初级胶囊层
        self.conv_caps = ConvCaps(32, 24, (1, 1), P, stride=(1, 1), iters=3)  # 卷积胶囊层
    ...
    def caps_reorder(self, imgcaps):
        ...
        h = imgcaps.size()[1]  # 高度
        w = imgcaps.size()[2]  # 宽度
        img_data = imgcaps.size()[3]  # 通道数
        num_imgcaps = int(img_data / (self.P * self.P))  # 胶囊数量
        
        # 分离姿态矩阵和激活值
        pose_range = num_imgcaps * self.P * self.P
        img_poses = imgcaps[:, :, :, :pose_range]  # 姿态矩阵部分
        img_acts = imgcaps[:, :, :, pose_range:pose_range + num_imgcaps]  # 激活值部分
    ...
    def forward(self, img, classification, concat_labels, epoch, thresh_ep):
        ...
        参数:
            img: 输入图像 (B, 3, T, H, W)
            classification: 分类标记 (B,)
            concat_labels: 标记连接指示 (B,)
            epoch: 当前epoch
            thresh_ep: 阈值epoch
            
        返回:
            out_1: 分割掩码 (B, 1, T, H, W)
            actor_prediction: 演员预测 (B, C)
            feat_shape: 特征形状 (B, 400, 24)
        """
        # 第一阶段特征提取
        x, cross56, cross112 = self.conv1(img)  # x:(B,832,1,28,28), cross56:(B,192,2,56,56), cross112:(B,64,4,112,112)
        
        # 3D Dropout
        x = self.dropout3d(x)  # (B,832,1,28,28)

        # 准备初级胶囊输入
        x = x.view(-1, 832, 28, 28)  # (B,832,28,28)
        cross28 = x.clone()  # 保存28x28特征
        x = self.primary_caps(x)  # (B,20,20,544)

        # 重新排列胶囊
        x = self.caps_reorder(x)  # (B,20,20,544)
        
        # 卷积胶囊层
        combined_caps = self.conv_caps(x)  # (B,20,20,408)

        # 解析胶囊输出
        h = combined_caps.size()[1]  # 高度
        w = combined_caps.size()[2]  # 宽度
        caps = int(combined_caps.size()[3] / ((self.P * self.P) + 1))  # 胶囊数量
        ranges = int(caps * self.P * self.P)  # 姿态矩阵总维度
        
        # 分离激活值和姿态矩阵
        activations = combined_caps[:, :, :, ranges:ranges + caps]  # (B,20,20,24)
        poses = combined_caps[:, :, :, :ranges]  # (B,20,20,384)
        actor_prediction = activations  # (B,20,20,24)
        
        # 准备特征形状输出
        feat_shape = activations  # (B,20,20,24)
        feat_shape = torch.reshape(feat_shape, (feat_shape.shape[0], feat_shape.shape[1]*feat_shape.shape[2], feat_shape.shape[3]))  # (B,400,24)
        
        # 计算演员预测(平均池化)
        actor_prediction = torch.mean(actor_prediction, 1)  # (B,20,24)
        actor_prediction = torch.mean(actor_prediction, 1)  # (B,24)
        ...
        poses = poses.permute(0, 3, 1, 2)  # (B,384,20,20)
        x = poses  # (B,384,20,20)

        # 上采样路径
        x = self.relu(self.upsample1(x))  # (B,64,28,28)
        x = x.view(-1, 64, 1, 28, 28)  # (B,64,1,28,28)

        # 跨层连接1
        cross28 = cross28.view(-1, 832, 28, 28)  # (B,832,28,28)
        cross28 = self.relu(self.conv28(cross28))  # (B,64,28,28)
        cross28 = cross28.view(-1, 64, 1, 28, 28)  # (B,64,1,28,28)
        x = torch.cat((x, cross28), dim=1)  # (B,128,1,28,28)

        # 上采样路径继续
        x = self.relu(self.upsample2(x))  # (B,64,2,56,56)
        
        # 跨层连接2
        cross56 = self.relu(self.conv56(cross56))  # (B,64,2,56,56)
        x = torch.cat((x, cross56), dim=1)  # (B,128,2,56,56)
        x = self.relu(self.upsample3(x))  # (B,64,4,112,112)

        # 跨层连接3
        cross112 = self.relu(self.conv112(cross112))  # (B,64,4,112,112)
        x = torch.cat((x, cross112), dim=1)  # (B,128,4,112,112)

        # 最终上采样
        x = self.upsample4(x)  # (B,128,8,224,224)
        
        # 最终3D Dropout
        x = self.dropout3d(x)  # (B,128,8,224,224)
        
        # 平滑输出
        x = self.smooth(x)  # (B,1,8,224,224)
        out_1 = x.view(-1, 1, 8, 224, 224)  # (B,1,8,224,224)
