...
import re
def iou(split, debug=False):
    """
    Calculates the accuracy, f-mAP, and v-mAP over the test set
    """
    from datasets.ucf_dataloader_eval import UCF101DataLoader
    pkl_file_label = "testing_annots.pkl"
    ...
    parser.add_argument('--txt_path', type=str,default='result.txt', help='Path to the txt file where the evaluation results will be saved.')
    ...
    def extract_number(filename):
        match = re.search(r'best_model_train_loss_(\d+)\.pth', filename)
        return int(match.group(1)) if match else -1

    # 找到所有匹配的权重文件
    saved_weights = glob.glob(osp.join(args.ckpt, 'best_model_train_loss_*.pth'))

    # 按数字大小从大到小排序
    sorted_weights = sorted(saved_weights, key=extract_number, reverse=True)

    for saved_wts in sorted_weights:
        ...
        with torch.no_grad():
            ...
            validationset = UCF101DataLoader('validation',[224, 224], 1, file_id="testing_annots.pkl", use_random_start_frame=False)
            ...
            for idx, sample in enumerate(tqdm(val_data_loader, desc=f'Evaluating {split} set', unit='video')):
            ...
            # output_file = os.path.join('ucf101-24_gradient_maps_results.txt')
            output_file = args.txt_path
            # 打开文件以追加模式写入结果
            with open(output_file, 'a') as f:
                # 将结果写入文件
                f.write(saved_wts+'\n')
                f.write(f'Accuracy: {n_correct / np.sum(n_vids):.4f}, IoU/fmap/vmap: {iou_threshs[4]:.2f} {fmAP[4]:.4f} {vmAP[4]:.4f}, {iou_threshs[10]:.2f} {fmAP[10]:.4f} {vmAP[10]:.4f}\n')
            print('Accuracy:', n_correct / np.sum(n_vids) , 'IoU/fmap/vmap', iou_threshs[4], fmAP[4], vmAP[4], iou_threshs[10], fmAP[10], vmAP[10])
