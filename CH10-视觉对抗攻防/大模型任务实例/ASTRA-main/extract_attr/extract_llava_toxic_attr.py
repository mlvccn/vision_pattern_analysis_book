...
for attack_type  in attack_types:
    ...
    data_query = []
    save_path = "./results/adv_img_attr/llava_toxic/{}".format(attack_type)
    for index, image in enumerate(image_list):
        for i in range(3):
            random_query = random.randint(0, len(harmful_behavior)-1)
            query = harmful_behavior[random_query]

            ImgAttr.init_image_query(image, query, num_ablations=32, ablation_keep_prob=0.50) 
            ImgAttr.partitioner.set_p(112)
    ...
    data.to_csv('./results/queries/llava_toxic_{}.csv'.format(attack_type), index=False)
