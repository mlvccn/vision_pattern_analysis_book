...
该方法来源于国际会议ECCV2022的 [Semi-supervised temporal action detection with proposal-free masking](https://arxiv.org/abs/2207.07059) 一文。
...
- Python == 3.7
- PyTorch == 1.9.0  **(请确保PyTorch版本至少为1.8)**
- NVIDIA GPU
- Kornia库
...
下载视频特征，并在 `config/anet.yaml` 文件中更新视频路径/输出路径。
...
要从头开始训练SPOT，请运行以下命令。可以通过修改 `config/anet.yaml` 文件来调整训练配置，包括预训练和微调阶段。
...
| 模型 | 数据集                   | mAP@0.5 |
| ---- | ------------------------ | ------- |
| SPOT | ActivityNet（10%有标记） | 48.6    |

## 致谢

我们想要感谢 [SPOT](https://github.com/sauradip/SPOT) 的作者们，他们的工作显著加快了我们书籍的开发进程。
