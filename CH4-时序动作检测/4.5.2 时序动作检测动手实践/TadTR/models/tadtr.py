...
from .local_attention import LocalContextAttention  # 导入新增的局部自注意力模块
...
class TadTR(nn.Module):
    ...
    def __init__(self, position_embedding, transformer, num_classes, num_queries, aux_loss=True, with_segment_refine=True, with_act_reg=True):
        """ 初始化模型。
        参数：
            backbone: 要使用的主干网络的torch模块。参考backbone.py
            transformer: 变形Transformer架构的torch模块。参考deformable_transformer.py
            num_classes: 动作类别数量
            num_queries: 动作查询数量（即检测槽位）。这是TadTR在单视频中能检测到的最大动作数。
            aux_loss: 是否使用辅助解码损失（各解码器层的损失）。
            with_segment_refine: 是否使用迭代段细化
        """
        super().__init__()
        ...
        # 新增局部注意力模块
        self.local_context_attention = LocalContextAttention(hidden_dim, num_heads=8, window_size=5)
    ...
    def _to_roi_align_format(self, rois, T, scale_factor=1):
        '''将RoIs转换为RoIAlign格式。
        参数：
            RoIs: 归一化的段坐标，形状为(batch_size, num_segments, 4)
            T: 视频特征序列的长度
        '''
    ...
    def forward(self, samples):
        """ 前向传播接收一个NestedTensor，其包含：
               - samples.tensors: 批量图像，形状为[batch_size x 3 x H x W]
               - samples.mask: 二进制掩码，形状为[batch_size x H x W]，填充像素位置为1
            或由张量和掩码组成的元组

            返回包含以下元素的字典：
               - "pred_logits": 所有查询的分类对数（包括无动作类）。
                                形状= [batch_size x num_queries x (num_classes + 1)]
               - "pred_segments": 所有查询的归一化段坐标，表示为（中心，宽度）。这些值在[0, 1]内归一化，
                               相对于单张图像的尺寸（忽略可能的填充）。
                               参考PostProcess获取未归一化段的方法。
        ...
        # 应用局部上下文注意力模块
        src_proj = self.input_proj[0](src)  # 形状：(batch_size, hidden_dim, T)
        src_proj = src_proj.permute(0, 2, 1)  # 调整为(batch_size, T, hidden_dim)以适配注意力模块
        local_features = self.local_context_attention(src_proj)  # 应用局部注意力（仅需特征输入）
        srcs = [local_features.permute(0, 2, 1)]  # 调整回原形状传入transformer
        masks = [mask]
