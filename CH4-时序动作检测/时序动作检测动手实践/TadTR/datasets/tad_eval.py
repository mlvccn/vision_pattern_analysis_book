...
class TADEvaluator(object):
    def __init__(self, dataset_name, subset, video_dict=None, nms_mode=['raw'], iou_range=[0.5], epoch=None, num_workers=None):
        ...
        if dataset_name == 'thumos14':
            ...
            anno_file = '/data16t/liujunyu/TadTR/thumos14/th14_annotations_with_fps_duration.json'
        ...
        for cls in range(num_classes):
            ...
            # 修改，在较新的 pandas 版本中，不再允许 drop() 使用位置参数指定 axis，而必须使用关键字参数
    ...
    def accumulate(self, test_slice_overlap=0):
        ...
        for nms_mode in self.nms_mode:
            ...
            # 修改
            self.pred_by_cls[nms_mode] = [self.all_pred[nms_mode][self.all_pred[nms_mode].cls == cls].reset_index(drop=True).drop('cls', axis=1) for cls in range(self.num_classes)]
