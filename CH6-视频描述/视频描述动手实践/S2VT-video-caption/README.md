...
该来源于国际会议ICCV2015的“[Sequence to Sequence -- Video to Text](https://arxiv.org/abs/1505.00487)”一文。
...
- MSVD 数据集：
  ...
  我也尝试过 ResNet152、VGG19、InceptionV4 等模型。你可以使用 `extract_features.py` 脚本提取你自己的特征。
...
步骤3：提取特征
  ...
  （你可能需要先创建 feat_path 目录，详情见 extract_features.py）

步骤4：准备字幕数据

  运行 prepare_captions.py。你可能需要在脚本中修改数据路径配置。
...
步骤6：测试 / 评估
  ...
  运行 eval.py。你可以选择使用 Beam Search 或常规方法进行生成。
...
步骤7：查看日志
  ...
  你可以在 ./runs 中查看 tensorboard 日志。运行命令：
