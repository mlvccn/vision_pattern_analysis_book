...
def evaluate(model, criterion, data_loader, device):
    ...
    with open('logs/log4', 'a') as log_file:
        log_file.write(f'* Clip Acc@1 {metric_logger.acc1.global_avg:.3f} Clip Acc@5 {metric_logger.acc5.global_avg:.3f}\n')
        log_file.write(f'* Video Acc@1: {total_acc:.4f}\n')
        log_file.write(f'* Class Acc@1: {str(class_acc)}\n')
...
def main(args):
    ...
    model.eval ()
    input=torch.randn(data_loader.batch_size, dataset.frames_per_clip, dataset.num_points, 3, device=device)
    flops, params = profile(model.module, inputs=(input,))
    with open('logs/log1', 'a') as log_file:
        log_file.write(f'Training time: {total_time_str}\n')
        log_file.write(f'Accuracy: {acc:.4f}\n')
        log_file.write(f'参数量: {params/1e6}M\n')
        log_file.write(f'GFLOPS: {flops/1e9}\n')

def parse_args():
    import argparse
    parser = argparse.ArgumentParser(description='P4Transformer Model Training')

    parser.add_argument('--data-path', default='data/MSR/point', type=str, help='dataset')
